...
OpenMotelRoomMenu = function(motelRoom)
    ...
    if cachedMotelRoom then
        for roomIndex, roomData in ipairs(cachedMotelRoom["rooms"]) do
            ...
            table.insert(menuElements, {
                ["label"] = allowed and "Entrar a la habitación de : " .. roomData["displayLabel"] .. "" or roomData["displayLabel"] .. "la habitacion está cerrada",
                ["action"] = roomData,
                ["allowed"] = allowed
            })
        end
    ...
    if #menuElements == 0 then
        table.insert(menuElements, {
            ["label"] = "No room is bought in this dorm."
        })
    end
...
OpenInviteMenu = function(motelRoomData)
    ...
    if #closestPlayers <= 0 then
        return ESX.ShowNotification("No hay nadie fuera de tu ~y~habitación~s~")
    end
    ...
    ESX.TriggerServerCallback("motel:retreivePlayers", function(playersRetreived)
        if playersRetreived then
            ...
            ESX.ShowNotification("No se pudo obtener información sobre los jugadores.", "error", 3500)
        end
    end, closestPlayers)
end
        ...
        for action, position in pairs(Config.MotelInterior) do
            ...
            if dstCheck <= 5.0 then
                ...
                if dstCheck <= 0.9 then
                    local displayText = "Músculo ~r~[E]~s~ " .. Config.ActionLabel[action] or action

                    --DrawScriptText(position, displayText)
                    coords = vector3(position.x,position.y,position.z+0.9)
                    ESX.ShowFloatingHelpNotification(displayText,coords)
...
OpenConfirmBox = function(motelRoom)
    ESX.UI.Menu.Open("default", GetCurrentResourceName(), "main_accept_motel", {
        ["title"] = "Do you wan't to buy #" .. motelRoom .. "?",
        ["align"] = Config.AlignMenu,
        ["elements"] = {
            {
        ...
        if action == "yes" then
            ESX.TriggerServerCallback("motel:buyMotel", function(bought)
                if bought then
                    ESX.ShowNotification("Tu compraste la habitación : #" .. motelRoom)

                    Cleanup()
                
                    cachedData["purchasing"] = false
                else
                    ESX.ShowNotification("No tienes dinero.")
                end
		...
		ESX.UI.Menu.Open("default", GetCurrentResourceName(), "motel_main_dressing_menu", {
			...
			TriggerEvent("skinchanger:getSkin", function(skin)
                ESX.TriggerServerCallback("motel:getPlayerOutfit", function(clothes)
                    ...
                    ESX.ShowNotification("Cambiaste tu indumentaria")
                end, currentOutfit)
			end)
        end, function(menuData, menuHandle)
			menuHandle.close()
    ...
    if insideMotel then
        ...
        if ownedMotel then
            ...
            ESX.Game.Teleport(PlayerPedId(), Config.LandLord["position"], function()
                ESX.ShowNotification("Saliste del servidor aqui ahora vuelves a estar aqui")
            end)
        end
